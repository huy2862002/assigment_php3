...
<body>
    <div class="container">
        <div class="login">
            ...
            <li class="login-box-msg text-danger"> {{session()->get('notExist')}}</li>
            @endif
            @if(session()->has('error_confirm'))
            <li class="login-box-msg text-danger"> {{session()->get('error_confirm')}}</li>
            @endif
            <form method="POST" action="{{route('web.auth.postForgot')}}">
                @csrf
                <div class="mb-3">
                    <label for="exampleInputEmail1" class="form-label"><b>Nhập email lấy lại mật khẩu</b></label>
                    <input type="text" name="email" class="form-control" placeholder="Email" value="{{old('email')}}">
                </div>
                <button type="submit" class="btn btn-success">Gửi Mã</button><br><br>
                         </form>
        </div>
